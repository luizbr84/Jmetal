package unirio.teaching.portfolio.general.calculation;

import java.io.FileWriter;
import java.io.IOException;
import java.io.PrintWriter;

import unirio.teaching.portfolio.general.model.Projeto;
import unirio.teaching.portfolio.general.model.ProjetosCandidatos;
import unirio.teaching.portfolio.general.model.Risco;
...
public class PortfolioCalculator
{
	...
	protected void salvaCorrelacoes(double[][] correlacoes, int numeroProjetos)
	{
		try 
		{
			FileWriter outFile = new FileWriter("c:\\correlacoes.txt");
	...
	private double[][] calculaCorrelacaoSpearman(int numeroCenarios, int numeroRiscos, int numeroProjetos, double[] probabilidadeCenarios, double[][] impactosCenarios) 
	{
		...
		try {
			  FileWriter outFile = new FileWriter("c:\\saida.txt");
			  PrintWriter out = new PrintWriter(outFile);
			  
			  for (int i = 0; i < numeroCenarios; i++)
			  {
				  for (int j = 0; j < numeroProjetos; j++)
					  out.print(df.format(impactosCenarios[j][i]) + "\t");
				  out.println();
			  }
			  out.close();
			  } catch (IOException e){
