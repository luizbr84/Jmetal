...
import jmetal.qualityIndicator.MaximumDistance;
...
public class MultiExperimentInstance
{
	...
	/**
	 * Retorna a distância máxima de cada ciclo da instância, dada a melhor frente de Pareto
	 */
	public double[] getMaximumDistances(ParetoFront bestFront)
	{
		double[] results = new double[cycles.size()];
		double[][] bestFrontValues = bestFront.getValues();
		...
		for (int i = 0; i < cycles.size(); i++)
		{
			MultiExperimentCycle cycle = getCycleIndex(i);
			double[][] currentFrontValues = cycle.getFront().getValues();
			results[i] = md.calculate(currentFrontValues, bestFrontValues, bestFront.getObjectiveCount());
		}
